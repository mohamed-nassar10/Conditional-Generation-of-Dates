...
Original file is located at
    https://colab.research.google.com/drive/10XiNczQYTwffW-UlA2H2qE0MMtdq0y28
"""
...
y = dnet(torch.randn(10, 8))
y.shape



def train(dnet, gnet, numepochs, batchsize):
    ...
    d_optimizer = torch.optim.Adam(model_des.parameters(), lr=0.01)
    g_optimizer = torch.optim.Adam(model_gen.parameters(), lr=0.01)
    

    
    
    
    trainLoss = torch.zeros(numepochs, 2)
    testLoss  = torch.zeros(numepochs, 2)
    trainAcc  = torch.zeros(numepochs, 2)
    testAcc   = torch.zeros(numepochs, 2)

    
    for epochi in range(numepochs):

        model_des.train() # switch to train mode
        model_gen.train() # switch to train mode
        ...
        for X,y in train_loader:
            ...
            yHat_Des = model_des(y)
            yHat_Gen = model_gen(X)
            ...
            # forward pass and loss for REAL
            pred_real   = model_des(y)                      # output of discriminator
            d_loss_real = lossfun(pred_real, real_labels)   # all labels are 1

            # forward pass and loss for FAKE
            fake_date   = model_gen(X)                         # output of generator
            pred_fake   = model_des(fake_date)                 # pass through discriminator
            ...
            # create fake and compute loss
            yHat      = model_gen(X)
            pred_fake = model_des(yHat)
            ...
            batchLoss.append([d_loss.item(), g_loss.item()])
            
            d1 = np.mean( (pred_real.cpu().detach().numpy() > 0.5) )
            d2 = np.mean( (pred_fake.cpu().detach().numpy() > 0.5) )
            ...
            batchAcc.append([d1, d2])


        trainLoss[epochi] = torch.tensor( np.mean(batchLoss, axis=0) )
        
        # trainAcc[epochi]  = 100*np.mean(batchAcc)
        # trainAcc[epochi]  = np.mean(batchAcc)
        trainAcc[epochi]  = 100 * torch.tensor( np.mean(batchAcc, axis=0) )

        



        model_des.eval() # switch to test mode
        model_gen.eval() # switch to test mode
        ...
        with torch.no_grad():
            ...
            # forward pass and loss for REAL
            pred_real   = model_des(yy)                     # output of discriminator
            d_loss_real = lossfun(pred_real, torch.ones(pred_real.size(0), 1).to(device))   # all labels are 1

            # forward pass and loss for FAKE
            yHat_test   = model_gen(XX)                        # output of generator
            pred_fake   = model_des(yHat_test)                 # pass through discriminator
            d_loss_fake = lossfun(pred_fake, torch.zeros(pred_real.size(0), 1).to(device))      # all labels are 0
            ...
            # # create fake and compute loss
            # fake_date = model_Gen(XX)
            # pred_fake = model_Des(fake_date)
            ...
            testLoss[epochi, 0] = d_loss
            testLoss[epochi, 1] = g_loss    
            ...
            testAcc[epochi, 0] = torch.mean( (pred_real.cpu() > 0.5).float() ).detach()
            testAcc[epochi, 1] = torch.mean( (pred_fake.cpu() > 0.5).float() ).detach()

        


        # if (epochi + 1) % 5 == 0:
        # print(f'Epoch {epochi+1}/{numepochs}, Train Loss: {trainLoss[epochi]:0.6f}, Test Loss: {testLoss[epochi]:0.6f}')
        print(f'Epoch {epochi+1}/{numepochs}, '
              f'Train Loss: [{trainLoss[epochi, 0].item():0.4f}] [{trainLoss[epochi, 1].item():0.4f}], '
              f'Test Loss: [{testLoss[epochi, 1].item():0.4f}] [{testLoss[epochi, 1].item():0.4f}]')
        


        
    return trainLoss.cpu(), testLoss.cpu(), trainAcc.cpu(), testAcc.cpu(), yHat.cpu(), y.cpu(), yHat_test.cpu(), yy.cpu()
...
checkpoint_half = torch.load('checkpoint_half.pth.tar', map_location=torch.device('cpu'))

# model.load_state_dict(checkpoint['model_state_dict'])
gnet.load_state_dict(checkpoint_half['model_state_dict'])

gnet(torch.rand((10, 10)).to(device))
...
trainLoss1, testLoss1, trainAcc1, testAcc1, yHat1, y1, yHat_test1, yy1 = \
                                                            train(dnet, gnet, numepochs=numepochs, batchsize=batchsize)

checkpoint_half = {'model_state_dict'      : gnet.state_dict(),
                  #'model_smote_state_dict': gnet_smote.state_dict(),
...
torch.save(checkpoint_half, 'checkpoint_half.pth.tar')
...
trainLoss1, testLoss1, trainAcc1, testAcc1
...
ax[0].plot(trainLoss1[:, 0], 'sb-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[0].plot(trainLoss1[:, 1], 'sy-', markerfacecolor='w', markersize=10, label='Generator')
...
ax[1].plot(testLoss1[:, 0], 'bo-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[1].plot(testLoss1[:, 1], 'yo-', markerfacecolor='w', markersize=10, label='Generator')
ax[1].plot(len(testLoss1[:, 1]) - 1, testLoss1[:, 1][-1], 'r*', markersize=15, label='Test')
...
# ax[2].plot(trainLoss1[:, 0], trainLoss1[:, 1], 'ko-', markerfacecolor='w', markersize=10)
ax[2].plot(trainLoss1[:, 0], trainLoss1[:, 1], 'k.', markerfacecolor='k', markersize=10, alpha=0.5)
# ax[2].plot(testLoss1, 'ko-', markerfacecolor='w', markersize=10, label='Test')

ax[2].set_xlabel('Discriminator loss')
ax[2].set_ylabel('Generator loss')
...
# ax[0].plot(smooth(trainLoss))
# ax[0].plot(smooth(testLoss))
ax[0].plot(trainAcc1[:, 0], 'sb-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[0].plot(trainAcc1[:, 1], 'sy-', markerfacecolor='w', markersize=10, label='Generator')
...
corrTrain = np.corrcoef(yHat1.detach().T, y1.T) [1,0]
ax[0].legend([ f'Train r={corrTrain:0.3f}'])
# ax[0].legend()



ax[1].plot(testAcc1[:, 0], 'bo-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[1].plot(testAcc1[:, 1], 'yo-', markerfacecolor='w', markersize=10, label='Generator')
ax[1].plot(len(testAcc1[:, 1]) - 1, testAcc1[:, 1][-1], 'r*', markersize=15, label='Test')
...
corrTest = np.corrcoef(yHat_test1.detach().T, yy1.T) [1,0]
ax[0].legend([ f'Test r={corrTest:0.3f}'])
# ax[0].legend()
ax[1].legend()



# ax[2].plot(trainAcc1, 'sb-', markerfacecolor='w', markersize=10, label='Train')
# ax[2].plot(testAcc1, 'ko-', markerfacecolor='w', markersize=10, label='Test')
# ax[2].plot(len(testAcc1) - 1, testAcc1[-1], 'r*', markersize=15, label='Test')
# ax[2].plot(trainAcc1[:, 0], trainAcc1[:, 1], 'ko-', markerfacecolor='w', markersize=10)
ax[2].plot(trainAcc1[:, 0], trainAcc1[:, 1], 'k.', markerfacecolor='k', markersize=10, alpha=0.5)
...
dnet_smote = discriminatorNet().to(device)
gnet_smote = generatorNet().to(device)



checkpoint = torch.load('checkpoint.pth.tar')
# model_smote.load_state_dict(checkpoint['model_smote_state_dict'])
gnet_smote.load_state_dict(checkpoint['model_smote_state_dict'])
...
trainLoss, testLoss, trainAcc, testAcc, yHat, y, yHat_test, yy = \
                                                train(dnet_smote, gnet_smote, numepochs=numepochs, batchsize=batchsize)
...
# ax[0].plot(smooth(trainLoss))
# ax[0].plot(smooth(testLoss))
ax[0].plot(trainLoss[:, 0], 'sb-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[0].plot(trainLoss[:, 1], 'sy-', markerfacecolor='w', markersize=10, label='Generator')
...
ax[1].plot(testLoss[:, 0], 'bo-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[1].plot(testLoss[:, 1], 'yo-', markerfacecolor='w', markersize=10, label='Generator')
ax[1].plot(len(testLoss[:, 1]) - 1, testLoss[:, 1][-1], 'r*', markersize=15, label='Test')
...
# ax[2].plot(trainLoss[:, 0], trainLoss[:, 1], 'ko-', markerfacecolor='w', markersize=10)
ax[2].plot(trainLoss[:, 0], trainLoss[:, 1], 'k.', markerfacecolor='k', markersize=10, alpha=0.5)
...
# ax[0].plot(smooth(trainLoss))
# ax[0].plot(smooth(testLoss))
ax[0].plot(trainAcc[:, 0], 'sb-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[0].plot(trainAcc[:, 1], 'sy-', markerfacecolor='w', markersize=10, label='Generator')
...
ax[1].plot(testAcc[:, 0], 'bo-', markerfacecolor='w', markersize=10, label='Discriminator')
ax[1].plot(testAcc[:, 1], 'yo-', markerfacecolor='w', markersize=10, label='Generator')
ax[1].plot(len(testAcc[:, 1]) - 1, testAcc[:, 1][-1], 'r*', markersize=15, label='Test')
...
# ax[2].plot(trainAcc[:, 0], trainAcc[:, 1], 'ko-', markerfacecolor='w', markersize=10)
ax[2].plot(trainAcc[:, 0], trainAcc[:, 1], 'k.', markerfacecolor='k', markersize=10, alpha=0.5)
...
def result_to_date(result):
    ...
    if int(result[:4]) < 1 or int(result):
        # print(result)
        return np.nan
...
def test_model(input_list):
    ...
    # print(torch.tensor(X_smote).shape)



    result       = gnet(torch.tensor(X).to(device).float())
    result_smote = gnet_smote(torch.tensor(X_smote).to(device).float())
...
# test_model(example_input.iloc[0].tolist())

# R.iloc[0], R_smote.iloc[0] = test_model(example_input.iloc[0].tolist())
...
checkpoint = {'model_state_dict'      : gnet.state_dict(),
              'model_smote_state_dict': gnet_smote.state_dict(),
